...
3. **Invoca al subagente `quality-guardian`** (via herramienta Agent) pasándole: la lista concreta de archivos a revisar, el contexto convencional cargado, y la instrucción de revisar únicamente ese alcance. Pídele evidencia concreta (`archivo:línea` + snippet) por hallazgo, priorizado: seguridad > integridad de datos > mantenibilidad > estilo.
